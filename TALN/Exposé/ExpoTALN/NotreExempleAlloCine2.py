...
for epoch in range(10):
    ...
    TestDataset = TensorDataset(torch.Tensor(Y_train_counts.toarray()), tensorSentimentTest)
    ...
    i = 0
    with torch.no_grad():
        for data2 in TestDataLoader:
            commentaire2, sentiment2 = data2
            resultat2 = ReseauNeuro(commentaire2.to(device))
            _, prediction = torch.max(resultat2.data, 1)
            AllPrediction.append(prediction.cpu().numpy())
            AllSentiment.append(sentiment2.cpu().numpy())
            if i % 10 == 0:
                pred = accuracy_score(prediction.cpu().numpy(), sentiment2.cpu().numpy())
                if pred == 1:
                    print("Prediction n°", i, " : ")
                    if FichierTest.sentiment[i] == 0:
                        print(FichierTest.commentaire[i], " --> Bon Commentaire")
                    else:
                        print(FichierTest.commentaire[i], " --> Mauvais Commentaire")
                else:
                    print("Prediction n°", i, " : ")
                    if FichierTest.sentiment[i] == 0:
                        print(FichierTest.commentaire[i], " --> Mauvais Commentaire")

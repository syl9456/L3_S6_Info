...
#
#   Fonction qui analyse le mot passé en paramètre
#
def analyseMot(mot):
...
if len(sys.argv) != 2:
    print("\tusage: py analyse_morphologique.py [Fichier de mot]")
    exit()
else:
    with open(sys.argv[1], "r") as fichMot:     # On ouvre le fichier des terminaisons
        for ligne in fichMot:                          # pour chaque ligne
            listeMot = ligne.split(" ")                # on créer une liste des mot séparé par un espace
            listeMot[len(listeMot)-1] = listeMot[len(listeMot)-1].strip('\n')     # On enlève les "\n"
            for unMot in listeMot:
                analyseMot(unMot)
